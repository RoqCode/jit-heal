...
export const LLMHeal = async (error: unknown, context: unknown) => {
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      instructions:
        "You generate minimal just-in-time fixes for runtime errors. Return only raw executable JavaScript code. Define exactly one function named heal with this signature: function heal(headerValue, config). Preserve the original function's behavior as much as possible. Fix only the failing condition shown by the error and context. Do not add aliases, fuzzy matching, broad normalization, new features, or behavior not required by the failing input. Do not use export, import, TypeScript types, Markdown fences, prose, explanations, headings, or surrounding text. The function must return one of config.available. If the context is insufficient, return the smallest safe function heal(headerValue, config) fallback.",
